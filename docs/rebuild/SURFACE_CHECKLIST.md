...
| Scene panel ▸ Summarise | Writes a summary. **Works with no API key** — the offline path is extractive, built from the scene's own strongest sentences, so every word is one the author wrote. With a provider it uses `completeDetailed` and says so if the reply was cut off | `29-progressions.spec.ts`, unit `progressions.spec.ts` |
| Scene panel ▸ summary stale chip | Fires when the **prose** changed after the summary was written. Reads `proseUpdatedAt`, never `updatedAt` — a POV dropdown cannot stale a summary | `29-progressions.spec.ts`, unit `progressions.spec.ts` |
| Scene panel ▸ What changed here | Progressions anchored to this scene, extraction's rows and the author's in one list | `29-progressions.spec.ts` |
| Scene panel ▸ Who or what / Replaces / Anchor here | Writes a `Progression`. "Replaces" picks a field (a replacement); left blank it is a free-prose addition. The field list is `generableFields`, the same one the drawer and the assembler use | `29-progressions.spec.ts` |
| Scene panel ▸ Remove progression | Deletes the row, audited and reversible | `29-progressions.spec.ts` |
| Editor `/progress ` | Opens the scene panel's composer with the entity picker focused. Deliberately leaves **nothing** in the document — a progression in the prose would reach the word count, the exports and extraction | `29-progressions.spec.ts` |
